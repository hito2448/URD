...
from dataset.visa import VisANoiseDataset, VisADataset
...
def train(device, classname, data_root, log, epochs, learning_rate, batch_size, img_size, iteration_i):
    ckp_path = './checkpoints_visa/' + classname + '.pth'
    # prepare data
    ...
    train_data = VisANoiseDataset(data_dir=train_dir, anomaly_source_path=anomaly_source_path, transform=transform,
                                     gt_transform=gt_train_transform, mask=mask, classname=classname)
    test_data = VisADataset(data_dir=test_dir, gt_dir=gt_dir, transform=test_transform, gt_transform=gt_transform)
...
def test(student, teacher, test_dataloader, device, log, save_max, epoch, ckp_path):
    ...
    with torch.no_grad():
        ...
        if save_max == []:
            ...
            if (auroc_px + auroc_sp + aupro) > (save_max[0] + save_max[1] + save_max[2]):
                save_max[0] = auroc_px
                save_max[1] = auroc_sp
                save_max[2] = aupro
                save_max[3] = ap_px
...
if __name__ == "__main__":
    ...
    classnames = ['candle', 'capsules', 'cashew', 'chewinggum', 'fryum', 'macaroni1', 'macaroni2', 'pcb1', 'pcb2',
                  'pcb3', 'pcb4', 'pipe_fryum']

    log = open("./log_train_visa.txt", 'a')
